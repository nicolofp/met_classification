...
N = 342
XM = rand(rng, Normal(0.0,0.2),N,55)
XC = rand(rng, Normal(0.0,0.2),N,2)
w = rand(Dirichlet(ones(55)))#vec([0.5 0.2 0.2 0.05 0.05])
a = 1.1
b = -0.23
d = vec([1.1 -0.12])
...
# k = 20.0
# a = 10*[1,1,1,1,1]#rand(Gamma(k,k),5)
# b = rand(Gamma(2.0,2.0))
# rand(DirichletLogit(a,b))
...
@model function bwqs_soft(cx, mx, y)
    ...
    # Set the priors on our coefficients.
    nfeatures = size(mx, 2)
    alpha = 50*ones(nfeatures) #~ filldist(Gamma(1.0,1.0), nfeatures)
    phi = 10 #~ Gamma(2.0,2.0)
...
mx3 = bwqs(XC, XM, y)
chain_mx1 = sample(mx1, NUTS(), 1000);#, thinning=10, discard_initial=2000);
#chain_mx1 = sample(mx1, NUTS(), MCMCThreads(), 2000, 4)
